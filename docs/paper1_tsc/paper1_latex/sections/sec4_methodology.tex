...
monotone functions of a single congestion signal---the per-lane halted-queue fraction $q_l$
...
transform of these two observed quantities (exact up to the per-lane-to-approach aggregation, the equal-group-size scaling of the
symmetric two-phase plans used here, and the boundary saturation). The actor's observation
therefore contains the approach-aggregated information that its pressure reward optimizes.
...
% [MAPPO training-loop pseudocode removed for length: the standard MAPPO loop with
%  per-agent GAE, a parameter-shared actor, and a centralized multi-head critic.]