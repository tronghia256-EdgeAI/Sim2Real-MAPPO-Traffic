...
no commodity roadside sensor can produce. This privilege dependence also makes a
direct empirical comparison ill-posed: re-implementing PressLight, MPLight, or
CoLight on camera-computable proxies would no longer faithfully represent them.
We therefore adopt IPPO~\cite{dewitt2020_ippo}---the strongest learned CTDE
baseline that runs on the \emph{identical} proxy observation and reward---as our
learned reference, and separately bound the advantage that any privileged-input
method could enjoy through a privileged-observation variant of our own policy
(the information-restriction cost of Section~VI-C).
