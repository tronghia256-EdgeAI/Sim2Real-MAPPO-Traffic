...
# after ~150k steps (~6-10h) run train_stage1_check_gate.sh; only if MAPPO
# beats max-pressure do we fan out the rest (train_stage2_n2_remaining.sh).
#
# The N3 stages carry no such risk, so they start immediately in parallel:
#   final2_main_ext   : n3_grid x {mappo, ippo} x seeds {2024..2028} @ 0.5M  (10 jobs)
#   final2_obsabl_03M : n3_grid x mappo x 3 obs-ablation arms x seeds {42,123,456} @ 0.3M (9 jobs)
#   final2_n2_05M     : n2_corridor x mappo x seed 42 @ 0.5M (gate job)
#
# Worker budget below assumes 56 vCPU. For a 32-vCPU VM use: MAIN=8 OBS=6 (N2 gate stays 1).
set -euo pipefail
cd "$(dirname "$0")/../.."
...
echo "[launch] final2_main_ext (10 jobs @ 0.5M, workers=${WORKERS_MAIN})"
...
echo "[launch] final2_n2_05M gate job (seed 42 only @ 0.5M)"
nohup python scripts/parallel_launcher.py \
    --campaign-id final2_n2_05M \
    --networks n2_corridor --algos mappo --obs-modes proxy \
    --seeds 42 \
    --total-timesteps 500000 --max-workers 1 \
    -- --no-libsumo \
...
grep -l "N2 Corridor detected" results/paper1_mappo/final2_n2_05M/*.log \
    && echo "[OK] N2 auto-enable confirmed in job log" \
    || echo "(not in log yet — check again shortly: grep 'N2 Corridor detected' results/paper1_mappo/final2_n2_05M/*.log)"

echo ""
echo "All launchers up. Monitor:  tail -f results/paper1_mappo/final2_*/campaign_manifest.json"
echo "Gate check after ~150k n2 steps (~6-10h):  bash scripts/final2/train_stage1_check_gate.sh"
